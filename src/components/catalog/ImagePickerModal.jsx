            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 border-b border-slate-100">
                    ...
                    <div className="flex gap-1 bg-slate-100 p-1 rounded-xl mt-4">
                        <button onClick={() => setMode('search')} className={`flex-1 py-2.5 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${mode === 'search' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>
                            <Search className="w-3.5 h-3.5" /> Buscar
                        </button>
                        <button onClick={() => setMode('url')} className={`flex-1 py-2.5 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${mode === 'url' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {mode === 'search' ? (
                        <div className="space-y-4">
                            ...
                            {/* Error with fallback */}
                            {error && (
                                <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl text-sm text-amber-700 space-y-3">
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-white border border-amber-300 text-amber-800 px-4 py-2.5 rounded-lg font-bold text-sm hover:bg-amber-100 transition-all flex items-center justify-center gap-2"
                                    >
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <button
                                    onClick={openGoogleImages}
                                    className="w-full bg-white border-2 border-slate-200 hover:border-indigo-400 hover:bg-indigo-50 text-slate-700 px-4 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 text-sm"
                                >
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    />
                                    <button onClick={handlePaste} className="px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-bold text-sm transition-colors">
                                        📋 Pegar
                                    </button>
                                </div>
                            </div>
