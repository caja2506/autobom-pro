...
export default function Sidebar() {
    ...
    const { proyectos, catalogo, engProjects, engTasks, managedLists, testConnection, setListManager } = useAppData();
        ...
        <>
            ...
            <aside className="hidden md:flex flex-col h-full bg-slate-900 text-white fixed left-0 top-0 z-[100] transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] w-16 hover:w-64 group border-r border-slate-800 shadow-[4px_0_24px_rgba(0,0,0,0.3)] hover:shadow-[12px_0_48px_rgba(0,0,0,0.5)] overflow-x-hidden overflow-y-hidden hover:overflow-y-auto scrollbar-thin">
                ...
                <nav className="flex-1 px-3 py-4 space-y-1">
                    ...
                    <button
                        onClick={() => setListManager({ isOpen: true, type: 'provider', title: 'Gestionar Proveedores' })}
                        title="Proveedores"
                        className="w-[220px] flex items-center gap-4 px-2.5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-all whitespace-nowrap"
                    >
                        <Truck className="w-5 h-5 flex-shrink-0 ml-[1px]" />
                        <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">Proveedores</span>
                        <span className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-[10px] bg-slate-800 px-2 py-0.5 rounded-full">{managedLists.providers.length}</span>
                    </button>
                    <button
                        onClick={() => setListManager({ isOpen: true, type: 'brand', title: 'Gestionar Marcas' })}
                        title="Marcas"
                        className="w-[220px] flex items-center gap-4 px-2.5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-all whitespace-nowrap"
                    >
                        <Tag className="w-5 h-5 flex-shrink-0 ml-[1px]" />
                        <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">Marcas</span>
                        <span className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-[10px] bg-slate-800 px-2 py-0.5 rounded-full">{managedLists.brands.length}</span>
                    </button>
                    <button
                        onClick={() => setListManager({ isOpen: true, type: 'category', title: 'Gestionar Categorías' })}
                        title="Categorías"
                        className="w-[220px] flex items-center gap-4 px-2.5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:bg-slate-800 hover:text-white transition-all whitespace-nowrap"
                    >
                        <LayoutList className="w-5 h-5 flex-shrink-0 ml-[1px]" />
                        <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">Categorías</span>
                        <span className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-[10px] bg-slate-800 px-2 py-0.5 rounded-full">{managedLists.categories.length}</span>
                    </button>
